...
def args_faster_rcnn_hoim(meta_parser=args_faster_rcnn):
    parser = argparse.ArgumentParser(
        add_help=False,
        parents=[meta_parser()],
        description='HOIM model based on Faster R-CNN')
    ...
    parser.add_argument('--gamma_r', dest='train.gamma_r',
                        default=2.0, type=float)
    parser.add_argument('--num_bg_size', type=int, default=5000,
                        help='Size of circular queue for background.')
    parser.add_argument('--use_vib', action='store_true', help='use VIB in reid embedding')
    parser.add_argument('--w_vib', dest='train.w_vib', default=1e-3, type=float)
