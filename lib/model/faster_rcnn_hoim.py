...
from .resnet_backbone import res2net_backbone
from .vib import VIB
from ..loss import HOIMLoss
from ..loss.ciou import compute_ciou
...
class FasterRCNN_HOIM(GeneralizedRCNN):
    ...
    def __init__(self, backbone,
        ...
        if rpn_anchor_generator is None:
            if anchor_scales is None:
                anchor_scales = ((38.4, 64, 128, 160, 256, 512),)
            if anchor_ratios is None:
                anchor_ratios = ((0.9, 1.4, 1.9, 2.3, 2.5, 2.7, 2.9, 3.1, 3.5, 4.0, 4.6, 5.2),)
        ...
        super(FasterRCNN_HOIM, self).__init__(
            backbone, rpn, roi_heads, transform, model_parallel=model_parallel)

    def _set_roi_heads(self, *args):
        return HOIMRoIHeads(*args)  # todo
    ...
    def ex_feat_by_roi_pooling(self, images, targets):
        ...
        proposals = [x['boxes'].to(self.model_parallel[0]) for x in targets]

        roi_pooled_features = self.roi_heads.box_roi_pool(
            self._od_move_helper(features, self.model_parallel[0]), proposals, images.image_sizes)
        rcnn_features = self.roi_heads.feat_head(roi_pooled_features.to(self.model_parallel[0]))
...
class HOIMRoIHeads(RoIHeads):
    ...
    def forward(self, features, proposals, image_shapes, targets=None):
        ...
        embeddings_, info_loss = self.embedding_head(rcnn_features)
        # print info_loss
...
def coord_regression_loss(class_logits, box_regression, labels, regression_targets):
    ...
    ciou_loss = compute_ciou(box_regression[sampled_pos_inds_subset, labels_pos],
        regression_targets[sampled_pos_inds_subset])
    ciou_loss = ciou_loss / labels.numel()
    box_loss = box_loss / labels.numel()
    n1 = nn.Parameter(torch.tensor(0.3))
    n2 = nn.Parameter(torch.tensor(0.7))
    # n1=0.2
    # n2=0.8
    box_loss = n1 * box_loss + n2 * ciou_loss
    return box_loss
...
class ReIDEmbeddingProj(nn.Module):
        ...
        for ftname, in_channel, indv_dim in zip(self.featmap_names, self.in_channels, indv_dims):
            if not self.use_vib:
                proj = nn.Sequential(
                    ...
                    MyBatchNorm1d(indv_dim))
                init.normal_(proj[0].weight, std=0.01)
                init.normal_(proj[1].bn2d.weight, std=0.01)
                init.constant_(proj[0].bias, 0)
                init.constant_(proj[1].bn2d.bias, 0)
            else:
                proj = VIB(in_channel, indv_dim, use_bn=True)
    ...
    def forward(self, featmaps):
        ...
        if len(featmaps) == 1:
            ...
            return F.normalize(torch.cat(outputs, dim=1)), info_loss
...
def get_hoim_model(pretrained_backbone=True,
    ...
    backbone, conv_head = res2net_backbone('res2net50', pretrained_backbone, kwargs["model_parallel"])
